...
function MonthlyDetailsContent() {
    ...
    const fetchDetails = async () => {
        setLoading(true);
        try {
            const res = await fetch(`/api/comerciales/detalles?asesor=${encodeURIComponent(asesor!)}&anio=${anio}&mes=${mes}`);
    ...
    const handleExportExcel = () => {
        ...
        XLSX.writeFile(wb, `Detalle_Asesor_${asesor?.replace(/ /g, '_')}_${anio}_${mes}.xlsx`);
    };

    if (!asesor || !anio || !mes) return <div className="p-8 text-center text-slate-500">Parámetros insuficientes</div>;
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
                <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h1 className="text-3xl font-black text-slate-900 tracking-tight">{asesor}</h1>
                            <p className="text-slate-500 font-bold flex items-center gap-2">
                                <span className="text-indigo-600 underline decoration-indigo-200">{MONTHS[parseInt(mes) - 1]} {anio}</span>
                                <span className="text-slate-300">|</span>
                                <span>Reporte Técnico de Pólizas (Asesor)</span>
                            </p>
                        </div>
                    </div>
