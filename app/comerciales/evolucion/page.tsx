    ...
    const chartOptions = {
        ...
        onClick: (_event: any, elements: any) => {
            if (elements.length > 0) {
                ...
                router.push(`/comerciales/evolucion/mes?asesor=${encodeURIComponent(asesor!)}&anio=${item.anio}&mes=${item.mes}`);
            }
        },
        scales: {
            y: {
    ...
    return (
        <div className="space-y-6">
                ...
                {!loading && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                        ...
                        <KPITooltip text="Ver listado de pólizas actualmente EN VIGOR.">
                            <div
                                onClick={() => router.push(`/polizas/listado?asesor=${encodeURIComponent(asesor!)}&estado=VIGOR`)}
                        ...
                        <KPITooltip text="Ver listado de pólizas actualmente en estado de SUSPENSIÓN.">
                            <div
                                onClick={() => router.push(`/polizas/listado?asesor=${encodeURIComponent(asesor!)}&estado=SUSPENSION`)}
                        ...
                        <KPITooltip text="Ver listado de pólizas producidas en este periodo que han sido anuladas.">
                            <div
                                onClick={() => router.push(`/polizas/listado?asesor=${encodeURIComponent(asesor!)}&estado=ANULADA&startYear=${startPeriod.year}&startMonth=${startPeriod.month}&endYear=${endPeriod.year}&endMonth=${endPeriod.month}`)}
            ...
            {selectedRamos.length > 0 && mixDepth === 'ramo' && (
                <div className="space-y-6 break-inside-avoid print:w-full">
                    {selectedRamos.map(ramo => {
                        ...
                        return (
                            <div key={ramo} className="bg-white p-8 print:p-6 rounded-2xl shadow-sm border border-purple-200">
                                <div className="flex items-center justify-between mb-6">
                                    <h2 className="text-xl font-bold text-slate-800 flex items-center gap-2">
                                        <PieChart className="w-5 h-5 text-purple-500" />
                                        Detalle del Ramo: <span className="text-purple-600">{ramo}</span>
                                    </h2>
                                    <button onClick={() => {
                                        const newSelection = selectedRamos.filter(r => r !== ramo);
                                        setSelectedRamos(newSelection);
                                        if (newSelection.length === 0) {
                                            setRamoPolizas([]);
                                            setRamoSearchTerm('');
                                        }
                                    }} className="px-4 py-1.5 rounded-md text-xs font-bold bg-slate-100 text-slate-600 hover:bg-slate-200 transition-all no-print">Cerrar ✕</button>
                                </div>
